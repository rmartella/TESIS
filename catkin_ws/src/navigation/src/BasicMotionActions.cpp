...
public:
	...
	void doGoalMotion(float goalX, float goalY, float goalTheta, bool correctAngle, bool moveBackwards, bool moveLateral, 
			float timeOut, bool &success, biorobotics::Polygon * polygons, int num_polygons);
	void doAngleMotion(float goalTheta, float timeOut, bool &success, biorobotics::Polygon * polygons, int num_polygons);
...
void BasicMotion::doGoalMotion(float goalX, float goalY, float goalTheta, bool correctAngle, bool moveBackwards, 
		bool moveLateral, float timeout, bool& success, biorobotics::Polygon * polygons, int num_polygons){
	ros::Rate rate(30);
	...
	while(ros::ok() && ((curr - prev).total_milliseconds() < timeout || timeout == 0) && !motionFinished){
		if(!correctFinalAngle){
			...
			bool testCollision = testAABBWithPolygons(currentX, currentY, currentTheta, 0.48, 0.48, polygons, num_polygons);
	    	std::cout << "testCollision:" << testCollision << std::endl;
...
void BasicMotion::doAngleMotion(float goalTheta, float timeout, bool& success, biorobotics::Polygon * polygons, int num_polygons){
	ros::Rate rate(30);
	tf::StampedTransform transform;
	float currentX, currentY, currentTheta;
	float errorAngle;
...
public:
	GoalDistAction(std::string name, BasicMotion *bm): as(nh, name, boost::bind(&GoalDistAction::executeCallback, this, _1),
	...
	void executeCallback(const common::GoalDistGoalConstPtr msg) {
	    ...
	    bm->doGoalMotion(goalX, goalY, goalTheta, false,dist < 0 ? true: false, moveLateral, timeout, success, 
	    		polygons_ptr, num_polygons);
...
public:
	GoalDistAngleAction(std::string name, BasicMotion * bm): as(nh, name, boost::bind(&GoalDistAngleAction::executeCallback, this, _1),
					false), action_name(name), bm(bm), polygons_ptr(0), num_polygons(0){
		as.start();
		envu.initRosConnection(&nh);
	}
	void executeCallback(const common::GoalDistAngleGoalConstPtr msg) {
		std::cout << "New Goal Dist angle:" << msg->dist << "," << msg->angle << std::endl;
		bool success;
		tf::StampedTransform transform;
		tf::TransformListener* tf_listener = bm->getTfListener();
	    tf_listener->lookupTransform("odom", "base_link", ros::Time(0), transform);
	    ...
	    bm->doAngleMotion(goalTheta, msg->timeout, success, polygons_ptr, num_polygons);
	    ...
	    if(success){
			...
			goalTheta = currentTheta;
			curr = boost::posix_time::second_clock::local_time();
			bm->doGoalMotion(goalX, goalY, goalTheta, false, msg->dist < 0 ? true: false, 
				false, msg->timeout == 0? 0: msg->timeout - (curr - prev).total_milliseconds(), success, polygons_ptr, num_polygons);
	    }
...
public:
	GoalPoseAction(std::string name, BasicMotion * bm): as(nh, name, boost::bind(&GoalPoseAction::executeCallback, this, _1),
					false), action_name(name), bm(bm) {
		as.start();
		envu.initRosConnection(&nh);
	}
	void executeCallback(const common::GoalPoseGoalConstPtr msg) {
		std::cout << "New Goal Dist angle:" << msg->pose.x << "," << msg->pose.y << "," << msg->pose.theta << std::endl;
		bool success;
		...
		bm->doGoalMotion(msg->pose.x, msg->pose.y, msg->pose.theta, true, false, false, msg->timeout, success, polygons_ptr, num_polygons);
		if(success)
	    	as.setSucceeded(result);
	}
private:
...
public:
	GoalPathAction(std::string name, BasicMotion * bm): as(nh, name, boost::bind(&GoalPathAction::executeCallback, this, _1),
					false), action_name(name), bm(bm) {
		as.start();
		envu.initRosConnection(&nh);
	}
	void executeCallback(const common::GoalPathGoalConstPtr msg) {
		...
		if(path.poses.size() > 0){
			...
			do{
				...
				bm->doGoalMotion(poseStmp.pose.position.x, poseStmp.pose.position.y, 0, false, 
					false, false, timeout == 0? 0: timeout - (curr - prev).total_milliseconds(), success, polygons_ptr, num_polygons);
				if(success)
					indexCurrPath++;
				rate.sleep();
				curr = boost::posix_time::second_clock::local_time();
			}while(ros::ok() && indexCurrPath < path.poses.size() && ((curr - prev).total_milliseconds() < timeout || timeout == 0) && success);
		}
		if(success)
	    	as.setSucceeded(result);
	}
...
int main(int argc, char ** argv){
	...
	GoalDistAngleAction goalDistAngleAction("goal_dist_angle_action", &basicMotion);
